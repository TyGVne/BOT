exports.run = async (message, args)=> {
  ...
  if ((!args[1]) || (!args[2]) || (!args[3]) ) 
    return  message.reply(":warning: :warning: **SAI CÚ PHÁP**:warning: :warning: \n *Tạo BlackList*:     `!MEDBAN [Link steam] [Số ngày blacklist (không quá 30 ngày)] [Lý Do]`");
  ...
  let medbanroomid = config.blacklistnganhdata["med"].banroomid
  let medbanchannel = client.channels.cache.get( medbanroomid);

  let medbanroomid2 = config.blacklistnganhdata["med"].banroom2id
  let medbanchannel2 = client.channels.cache.get( medbanroomid2);

  let medbanlogid = config.blacklistnganhdata["med"].banlogid
  let medbanlogchannel = client.channels.cache.get( medbanlogid);
  ...
  await MongoClient.db("bannganh").collection("medban").insertOne({"steamid" : `${steamid}` , "name" : `${name}`, "username" : `${username}` ,  "reason" : `${lydo}` , "timestart" : new Date(timestart) , "timeend" :  new Date(timeend)  })
  ...
  medbanlogchannel.send("**" + name + "** đã Ban **" + username + "** `(ID steam: "+ steamid + ")` thời gian: `" + timestart +" - "+ timeend + "` ( Lý Do: *" + lydo + "*). - `" + timenow + '` ');
